...
# https://github.com/nvie/gitflow/blob/master/Makefile

# Licensed under the same restrictions as git-flow:
# https://github.com/nvie/gitflow/blob/develop/LICENSE
    ...
    case "$1" in
        ...
        *)
            ...
            if [ -f "$REPO_NAME/$SUBMODULE_FILE" ] ; then
                ...
                cd "$REPO_NAME"
                git submodule init
                echo "-- Fixing protocol error in submodule URL"
                sed -i 's/url = git:\/\//url = https:\/\//' .git/config
